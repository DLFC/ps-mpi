  ...
  void operator()(const KVMeta& req_meta, const KVPairs<Val>& req_data, KVServer<Val>* server) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (req_meta.push) {
        ...
        // PS_VLOG(1) << "IN [Handle] [push]: " << key << "->" << store[key];
      } else {
        // PS_VLOG(1) << "IN [Handle] [pull]: " << key << "->" << store[key];
        res.vals[i] = store[key];
      }
    }
    // PS_VLOG(1) << "IN [Handle]: " << res.keys << res.vals;
...
void KVServer<Val>::Process(const Message& msg) {
  ...
  if (n) {
    ...
    // PS_VLOG(1) << "[Process push]: " << SArray<uint64_t>(msg.data[0]) << "==" << data.keys;
...
void KVServer<Val>::Response(const KVMeta& req, const KVPairs<Val>& res) {
  ...
  if (res.keys.size()) {
    ...
    // PS_VLOG(1) << "IN [Response]: " << res.keys << res.vals;
    if (res.lens.size()) {
...
void KVWorker<Val>::Send(int timestamp, bool push, int cmd, const KVPairs<Val>& kvs) {
  ...
  for (size_t i = 0; i < sliced.size(); ++i) {
    ...
    if (kvs.keys.size()) {
      ...
      // PS_VLOG(1) << "IN [PULL]: " << kvs.keys << kvs.vals;
      if (kvs.lens.size()) {
